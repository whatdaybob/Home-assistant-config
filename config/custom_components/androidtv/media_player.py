...
# Fallback images
APP_IMAGE_URL_BASE = "/local/community/androidtv/{0}.png"
...
class ADBDevice(MediaPlayerDevice):
    ...
    async def async_get_media_image(self):
        ...
        if not self._screencap:
            try:
                appImageURL = APP_IMAGE_URL_BASE.format(self.app_name.casefold())
                resp = requests.head(self.hass.config.api.base_url + appImageURL)
                if resp.status_code == RESPONSE_OK:
                    return appImageURL, "image/png"
